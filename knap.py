...
def calculate_profit(profit_matrix, weights, profits, item_index, current_capacity):
...
def solve_knapsack(profits, weights, capacity):
    ...
    for i in range(1, n_items+1):
        for j in range(capacity+1):
            profit_matrix[i][j] = calculate_profit(profit_matrix, weights, profits, i, j)
...
if __name__ == "__main__":
    ...
    profit_matrix, max_profit = solve_knapsack(profits, weights, capacity)
    items_included = find_items(profit_matrix, weights)
    print(max_profit)
    print(items_included)